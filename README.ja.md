...
- デフォルトのアイコン表示には [Nerd Font](https://www.nerdfonts.com/) **v3** が必要。無くても壊れることはなく、アイコンが豆腐（□）になるだけで、情報はすべてテキスト（モデル名、パス、％、ブランチ名）として読める。✓ ✎ ⚠ などの状態記号は標準 Unicode なので、現代的なフォントならどれでも表示される
